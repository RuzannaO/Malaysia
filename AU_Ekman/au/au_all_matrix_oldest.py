...
def AU_10_12(landmarks, points_10, points_12_right, points_12_left, points_12_middle, window_parameter, au_12_low, au_12_high, au_10, list_of_au_current_frame):
    ...
    elif (np.mean(points_12_left)/left_12 > au_12_high): #and (np.mean(points_12_middle)/middle_12 < au_12_middle):
        list_of_au_current_frame += dict_["AU_12"]
        list_of_au_current_frame += dict_["AU_12_uni"]
...
def AU_43_45(landmarks, COUNTER, EYE_AR_THRESH, EYE_AR_CONSEC_FRAMES_BLINK, EYE_AR_CONSEC_FRAMES_CLOSE, fast_blinking, gazing, window_parameter_fast_blink_gaze, list_of_au_current_frame, norm_blinking_freq):
    ...
    if sum(fast_blinking) > norm_blinking_freq:
        # list_of_au_current_frame.append(au_45_text[1])
        # print("fast_blinking")
        list_of_au_current_frame += dict_["FAST_BLINKING"]

    if sum(gazing) <= 15:
        # list_of_au_current_frame.append(au_45_text[2])
        # print("gazing")
...
def AU_M67_M68_75_76(gray, landmarks, au_list_eye, ratio_1_left, ratio_2_up, ratio_1_right, ratio_2_down,
    ...
    # cv2.imshow("roi" + eye_, roi)
    roi = cv2.equalizeHist(roi)
    # cv2.imshow("roi_norm" + eye_, roi)
    ...
    for cnt in contours:
        ...
        area = w*h
        if area/np.mean(points_area) > area_parameter:
        ...
        dist_2, _ = get_distance_points([landmarks.part(p1).x, landmarks.part(p1).y], [landmarks.part(p2).x, landmarks.part(p2).y])
        ratio_1 = dist_1/dist_2
        if ratio_1 < ratio_1_left:
            _, dist_3 = get_distance_points([landmarks.part(p3).x, landmarks.part(p3).y], centre_)
            _, dist_4 = get_distance_points([landmarks.part(p3).x, landmarks.part(p3).y], [landmarks.part(p4).x, landmarks.part(p4).y])
            ...
            _, dist_3 = get_distance_points([landmarks.part(p5).x, landmarks.part(p5).y], centre_)
            _, dist_4 = get_distance_points([landmarks.part(p5).x, landmarks.part(p5).y], [landmarks.part(p6).x, landmarks.part(p6).y])
            ratio_2 = dist_3/dist_4
            if ratio_2 < ratio_2_up:
            ...
            _, dist_3 = get_distance_points([(landmarks.part(p5).x + landmarks.part(p3).x)/2, (landmarks.part(p5).y + landmarks.part(p3).y)/2], centre_)
            _, dist_4 = get_distance_points([(landmarks.part(p5).x + landmarks.part(p3).x)/2, (landmarks.part(p5).y + landmarks.part(p3).y)/2],
                                        [(landmarks.part(p4).x + landmarks.part(p6).x)/2, (landmarks.part(p4).y + landmarks.part(p6).y)/2])
            ratio_2 = dist_3/dist_4
            if ratio_2 < ratio_2_up:
